...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, transform, data, apply_multiply=False, **kwargs):
        super(Dataset, self).__init__()

        self.transform = transform
        self.sentences = transform.load(data, **kwargs)
        if apply_multiply:
            rate = kwargs.get('data_multiply', 1)
            assert rate >= 1 and isinstance(rate, int)
            self.sentences = self.sentences * rate
