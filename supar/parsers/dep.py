...
class BiaffineDependencyParser(Parser):
    ...
    def _train(self, loader, unsup_loader=None):
        self.model.train()

        bar, metric = progress_bar(loader), AttachmentMetric()
        unsup_iter = iter(unsup_loader) if unsup_loader is not None else None
        ...
        for i, batch in enumerate(bar, 1):
            ...
            if self.contrastive_training and self.contrastive_loss_weight > 0:
                if unsup_loader is not None:
                    try:
                        words, texts, *feats, _, _ = next(unsup_iter)
                    except StopIteration:
                        unsup_iter = iter(unsup_loader)
                        words, texts, *feats, _, _ = next(unsup_iter)
                loss_cntr = self.model.constrastive_loss(words, feats)
                loss_cntr = self.contrastive_loss_weight * loss_cntr
                loss_orig = loss
                loss = loss_orig + loss_cntr
            ...
            if self.contrastive_training and self.contrastive_loss_weight > 0:
                bar.set_postfix_str(f"lr: {self.scheduler.get_last_lr()[0]:.4e} - "
                                    f"loss: {loss:.4f} (c: {loss_cntr:.4f}) - {metric}")
            else:
