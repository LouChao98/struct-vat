...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        if encoder != 'bert':
            ...
            if normalize_emb:
                print("untraced modification. elementwise_affine=False.")
                self.emb_layernorm = nn.LayerNorm(n_input, elementwise_affine=False)
        if encoder == 'lstm':
    ...
    def embed(self, words, feats):
        ...
        if self.args.normalize_emb:
            embed = self.emb_layernorm(embed)

        return embed

    def encode(self, words, feats=None, embed=None):
        if self.args.encoder == 'lstm':
            embed = self.embed(words, feats) if embed is None else embed
            x = pack_padded_sequence(embed, words.ne(self.args.pad_index).sum(1).tolist(), True, False)
            x, _ = self.encoder(x)
